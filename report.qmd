...
What makes this dataset in particular interesting is the levels in the SWC dataset. The SWC is measured at 4 different depts in the soil (suurface, 0.3m, 0.5m, and 1m below). While I didnt get time to look at this in this project, a goal is to look at the behaviour of SWC at 4 levels, and look for example for cross correlation in that data. 
...
def create_seasonal_plot(df, station):
    ...
    for level in station_data['level'].unique():
        ...
        fig.add_trace(go.Scatter(
            ...
            y=monthly_data['mean'],
            mode='lines',
            name=f'{station} - Level {level} Mean',
            line=dict(width=2)
        ))
    ...
    fig.update_layout(
        title=f'Seasonal Variation of SWC - Station {station}',
        xaxis_title='Month',
        yaxis_title='SWC (%)',
        xaxis=dict(tickmode='array', tickvals=list(range(1, 13)), ticktext=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']),
        legend_title='',
...
# First, let's convert the dictionary to a DataFrame
df_list = []
for key, value in reco_swc_filtered.items():
    station = key.split('_')[0]
    level = key.split('_')[1]
    df = pd.DataFrame(value)
    df['station'] = station
