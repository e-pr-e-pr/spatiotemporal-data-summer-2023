...
All the data used in this analysis is from FLUXNET - here is the link straight to the data download page: [FLUXNET data download](https://fluxnet.org/data/download-data/) . The data is free to download, but a user account must be made before the data can be downloaded. 

I downloaded the data via this website with point-and-click; I did not use a download script. Since the data is stored in .csv format, it is pretty light, and point-and-click worked just fine and took only about 20 minutes. :) I did some data processing to it - taking the averages. The code for that is also found below. I checked the daily data from FLUXNET, and this is different from the daily average - it just takes one data point and calls it a day. I wanted the daily average, hence this process. For RECO, I used the RECO_DT variable. For SWC, the variables are called SWC_1, SWC_2, SWC_3, and SWC_4. 


This is the steps/things I did to download the data:

- Navigate to [FLUXNET data download](https://fluxnet.org/data/download-data/) 

- Select the FLUXNET-CH4 Community Product - FLUXNET-CH4 CC-BY-4.0 Data

- Select all the sites, and fill in the intended use section of the page

- Download all files - consider using a plugin like DownThemAll (FireFox, Chrome) to download all the files at once.

- Unzip all the folders and put them where you want (consider using 7zip or similar to do them all at once)

- Run the following code to process the hourly data into daily data

```{python}

def process_csv_file(file_path, column_to_analyse, output_dir):
    """
    Processes a CSV file to calculate daily averages and counts for a specified column.
    
    Parameters:
        file_path (str): Path to the input CSV file.
        column_to_analyse (str): The column for which to calculate daily averages. i.e. SWC_1, SWC_2, or SWC
    ...
    column_parts = column_to_analyse.split('_')
    column_base = column_parts[0].lower()
    
    #check if there's level number after the underscore
    if len(column_parts) > 1 and column_parts[1].isdigit():
        level = column_parts[1]
        output_file = f"{station_name}_{column_base}_l{level}_daily_avg.txt"
    else:
        output_file = f"{station_name}_{column_base}_daily_avg.txt"
    
    df = pd.read_csv(file_path, parse_dates=['TIMESTAMP_START'], date_parser=lambda x: pd.to_datetime(x, format='%Y%m%d%H%M'))
    ...
    #calculate daily averages and count nobs
    daily_stats = df.groupby(df.index)[column_to_analyse].agg(['mean', 'count'])
    
    output_path = os.path.join(output_dir, output_file)
    
    with open(output_path, 'w') as f:
        f.write(f"Daily averages and observation counts for {column_to_analyse}\n")
        f.write("time,value,nobs\n")  # nobs = number of observations
        for date, row in daily_stats.iterrows():
            f.write(f"{date},{row['mean']:.4f},{row['count']}\n")
    
    print(f"Results have been written to {output_path}")
```
Now you should have folders with individual stations and levels as .txt files. This is also the data you will find here in this repo, under /data/RECO/ or /data/SWC/.

[FLUXNET data variables](https://fluxnet.org/data/aboutdata/data-variables/) and [FLUXNET data product information])https://fluxnet.org/data/fluxnet2015-dataset/fullset-data-product/)  These pages provides an overview of the data variables found in the data. 

For the station location coordinate data, I got this also from FLUXNET. They didnt have it available to download, but they did have it as a table on the following page: [FLUXNET station locations](https://fluxnet.org/sites/site-list-and-pages/). I simply copy and pasted this table into a .csv file to use here in my analysis. 
        ...
        for current_level in level_keys[1:]:
            ...
            while i < len(time_data[current_level]):
                ...
                if timestep not in time_data[reference_level]:
                    ...
                    # rmove the timestep from the list
                    del time_data[current_level][i]
                else:
                    i += 1  # move to the next timestep

            # update dictionary with correect time data
...
```
The following function plots the results of the above metric claculation visually, on an interactive world map. It uses Plotly, which is an awesome module but pretty complicated. I cant explain everything for it here; I refer the interested reader to the official Plotly website for more information on everything used here! [Plotly official page](https://plotly.com/python/getting-started/#overview). Also useful are the following : [Plotly tutorial](https://www.geeksforgeeks.org/python-plotly-tutorial/)
            ...
            dropdown_buttons.append(
                dict(
                    ...
                    args = [{'visible': visible},
                            {'annotations[1].text': f'{metric.capitalize()} between SWC (%) and {data_type} ("gC m-2 day-1") for Level {level}, all stations'}]
                )
            )
...
def create_timeseries_plot_single_level(data, data_type):
    ...
    fig.update_layout(
        ...
        title_x=0.5,
        xaxis_title="Time",
        yaxis_title=f"{data_type.upper()} (gC m-2 day-1)"
    )
...
This is how I filtered the full station coordinate data set for only the ones that were present in my final datasets. This wont run this in the final reproducible workflow, since I just loaded the CSV I already filtered- but Ive include but Ive included it as a snippet for full reproducibility.
...
#esure latitude and longitude are numeric
...
I now visualise the results of the metric calculations. The visual representation on a map creates an instructive first look at the potential patterns in the data. Ive displayed it as a dropdown menu, to save some space on the page, and to easily find the variable one might be interested in. So initial thoughts on what the results could mean: 
...
#merge location data with metrics based on the station
...
Here, I will focus on the correlation coefficient and R sqaured values. One can see that in both the RECo and logRECO with SWC metrics, there are similar values (RECO / logRECO and SWC - in both cases correlation: They all err to the negative, with very weak positive correlaation only sometimes. R squared: 0 to between 0.3 and 0.4). This suggests that the relationship  between RECO and SWC is highly non linear. Simple transformations may not be sufficient to capture the complexity of the relationship, and indeed might involve more complex interactions and threshold effects that arent easily captued by a mathematical transformation. Additionally, it confirms the fact that it is highly likely that SWC and RECO are influenced by more factors - this multidimensional relationsip cant be reduced to these 2 variables alone. 

In particular the low R squared values indicate, as briefly mentioned, that the linear relationship between SWC and RECO is generally weak, and that SWC explains at most 0.3 - 0.4 (level and station dependent) of the variability in RECO. It is highly likely that other factors such as temperature, available nutrients, vegetation type, precipitation etc...play a role in determining RECO. 

The predominantly negative correlation is easier to understand, especially when you consider that most of our data is in Northern Europe - a part of the world that experiences quite large seasonal differences, and only limited parts of the year where things can actually grow. In European winter, which tend to be wet and cold,  soil water content increases, while RECO decreases (due to cooler temperatures, lack of sunlight and available nutirents). In summer in Europe, which tend to be warm and not quite so wet, soil water content decreases (due to less rainfall and higher temperatures), and RECO increases. 

If we look at the only station outside of Europe - GF-Guy (Guyana, South America), we see a very, very weak correlation that fluctuates between positive and negative. This is effectively no correlation between the 2. Guyana has a tropical climate classification, with generally warm, wet weather and a pretty stable temperature year round. The R squared for each level in both examples is prqactically 0 (0.013, for example), indicating that SWC explains basically no variability of RECO; the variability must come from other factors. This pattern is probably due to the relative stability of Guyana's climate: SWC might not be a limiting factor of biological respiration processes in such places due to the relative abundance of warmth and rain. 

There does not seem to be a clear pattern with depth, either. 

Its clear from these results that the range of correlation coefficients and R squared values suggests large variabilty across different ecosystems and time periods, which could be due to differences in soil type, vegetation cover, climate zones. More data, especially from different climates, is necessary to confirm this. 

It must also be noted that the GF-Guy station is one of the stations that has less, and older data, than some others. I trust this data, but I do think it is important to point out that I am not working with the same amount of data points for every station. This would of course be the ideal case, but I am limited by available data. 
...
In these temporal plots of SWC, I observestrong seasonality, but no real consistency in the levels - in different stations, different level numbers are feature te highest and the lowest SWC. This suggests that soil structure and land cover type plays a role in SWC.
## Summary of results

- The relationship between Soil Water Content (SWC) and Ecosystem Respiration (RECO) is highly non-linear, with predominantly negative correlations and low R-squared values, suggesting that SWC alone explains only a small portion of RECO variability.
- Significant differences were observed between European and tropical sites, with the tropical station in Guyana showing almost no correlation between SWC and RECO, likely due to its stable climate.
- Temporal plots revealed strong seasonality in SWC and RECO for European stations, but with inconsistent patterns across different soil depths, indicating the influence of local soil structure and land cover.
- The analysis of monthly seasonal variations highlighted recurring patterns and fluctuations, which could be crucial for predicting future values and understanding climate change impacts.
- The study underscores the complexity of ecosystem processes and the need for incorporating additional variables such as temperature, precipitation, and soil characteristics to fully understand the dynamics between SWC and RECO.
