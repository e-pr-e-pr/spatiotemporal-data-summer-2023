...
def create_correlation_map(df, metric='correlation'):
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=("Level 1", "Level 2", "Level 3", "Level 4"),
    ...
    # Define the layout for each subplot
    layout = dict(
        mapbox=dict(
    ...
    for i, level in enumerate(['l1', 'l2', 'l3', 'l4'], 1):
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                text=level_data.apply(lambda row: f"Station: {row['station']}<br>{metric.capitalize()}: {row[metric]:.2f}<br>R-squared: {row['r_squared']:.2f}<br>Slope: {row['slope']:.2f}<br>Intercept: {row['intercept']:.2f}", axis=1),
                hoverinfo='text'
            ),
            row=(i-1)//2 + 1,
            col=(i-1)%2 + 1
    ...
    fig.update_layout(
        height=1000,
        width=1000,
        title_text=f"Map of {metric.capitalize()} between RECO and SWC for Different Levels",
        **layout
    )
...
I now visualise the results of the metric calculations. The visual representation on a map creates an instructive first look at the potential patterns in the data. The full table of the regression metrics between the levels in SWC stations, and te RECO can be found in the following table. The following plot is an example of a visual representation one could do to visual spatial patterns. (I tried to get it to work for all stations, but the loops and functions required just were not working for me :( )

```{python}

# Usage:
fluxnet_locations = pd.read_csv(filtered_station_locations)

# Prepare data for plotting
plot_data = []
for station, levels in swc_reco_four_levels_metrics.items():
    station_name = station.split('_')[0]
    level = station.split('_')[-1]  # Extract level from station key
    station_info = fluxnet_locations[fluxnet_locations['station'] == station_name]
    ...
    metrics = levels[station_name]  # Access metrics directly
    plot_data.append({
        ...
        for key, values in station_data.items():
            if key != 'time':
                fig.add_trace(go.Scatter(
                    ...
                    y=values,
                    mode='lines',
                    name=f'{station_key} - {key}'
                ))
    
    fig.update_layout(
        title=f'Soil Water Content - Station {prefix}',
        xaxis_title='Time',
        yaxis_title='Soil Water Content',
...
def plot_grouped_stations(prefix, grouped_data):
    ...
    fig = make_subplots(rows=1, cols=1, 
                        subplot_titles=[f'Soil Water Content - Station {prefix}'])
    ...
    fig.update_layout(
        ...
        title_text=f"Soil Water Content for Station {prefix}",
        showlegend=True,
...
```{python}
# make csv file into data frame for easy access
fluxnet_locations = pd.read_csv(filtered_station_locations)
valid_prefixes = set(fluxnet_locations['station'])  
...
for prefix, grouped_data in grouped_stations.items():
    fig = plot_grouped_stations(prefix, grouped_data)
    fig.show()
...
These plots are very messy, and a lot of them feature sections with very flat lines. These are simply points where, for a period of time, data at that level was not recorded for whatever reason. Even though this raw data is quite messy, we can still see 2 interesting things - the beaviour of SWC at different depths, and, in many cases, a strong seasonality. So then decided to look at the seasonal trends. Again, I wasnt able to create the plots in the way I wanted to for this (it would have been many many pages long), so I have included here 2 example plots. I chose the stations FR-Hes, and ANOTHER, since the have a long time series, and so are the most optimal for demonstrating seasonal variation with this dataset. 



```{python}
fr_hes_data = {key: value for key, value in swc_filtered.items() if key.startswith('FR-Hes')}

# Create a function to process the data and calculate monthly averages
def process_level_data(data):
    df = pd.DataFrame(data)
    df['time'] = pd.to_datetime(df['time'])
    df['month'] = df['time'].dt.month
    monthly_avg = df.groupby('month')['data'].agg(['mean', 'std']).reset_index()
    return monthly_avg

# Process data for each level
levels_data = {}
for key, value in fr_hes_data.items():
    level = key.split('_')[-1]  # Extract level from the key
    levels_data[level] = process_level_data(value)

# Define colors for each level
colors = ['blue', 'green', 'red', 'purple']

# Create the plot
fig = make_subplots(rows=2, cols=2, subplot_titles=["Level 1", "Level 2", "Level 3", "Level 4"])

for i, (level, data) in enumerate(levels_data.items(), 1):
    row = (i-1) // 2 + 1
    col = (i-1) % 2 + 1
    ...
    fig.add_trace(
        go.Scatter(
            x=data['month'].tolist() + data['month'].tolist()[::-1],
            y=(data['mean'] + data['std']).tolist() + (data['mean'] - data['std']).tolist()[::-1],
            fill='toself',
            fillcolor=f'rgba({i*50},0,255,0.1)',
            line=dict(color='rgba(255,255,255,0)'),
            name=f'{level} Std Dev',
            showlegend=False
        ),
        row=row, col=col
    )
...
fig.update_layout(
    height=800,
    width=1000,
    title_text="Seasonal Variation of SWC for FR-Hes",
    showlegend=True
)

for i in range(1, 5):
    fig.update_xaxes(title_text="Month", row=(i-1)//2+1, col=(i-1)%2+1, tickvals=list(range(1, 13)), ticktext=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'])
    fig.update_yaxes(title_text="SWC (%)", row=(i-1)//2+1, col=(i-1)%2+1)
