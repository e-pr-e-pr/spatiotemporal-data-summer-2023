...
from plotly.subplots import make_subplots # for dropdown menu
from IPython.display import display, HTML # to elp the matplotlib plots render properly
...
We can now look at te temporal evolution of SWC and RECO at the different stations. This function filters for the stations with only 4 levels present, and plots them - with each plot featuring the 4 levels. 
...
fluxnet_locations = pd.read_csv(filtered_station_locations) # csv to dataframe for easier processing
...
```{python}
# plotting the mean seasonal variation of each station, as well as the standard deviation, first for SWC

# Assuming swc_filtered is your dictionary with the data
# First, let's convert the dictionary to a DataFrame
df_list = []
for key, value in swc_filtered.items():
...
for station in stations:
    fig = create_seasonal_plot(df, station)
    fig.show()
```

And the same thing for RECO:
ADD FUNCTION DESCRIPTION
```{python}
def create_combined_interactive_plot(df, stations, featured_stations):
    # Create separate subplots for each featured station
    fig = make_subplots(rows=len(featured_stations), cols=1, 
                        subplot_titles=[f"Seasonal Soil Water Content - {station}" for station in featured_stations])
    
    # Add traces for featured stations
    for i, featured_station in enumerate(featured_stations, start=1):
        station_data = df[df['station'] == featured_station]
        
        for level in station_data['level'].unique():
            level_data = station_data[station_data['level'] == level]
            
            # Group by month and calculate mean and std
            monthly_data = level_data.groupby(level_data['time'].dt.month).agg({'data': ['mean', 'std']})
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=monthly_data['mean'],
                    mode='lines',
                    name=f'{featured_station} - {level}',
                    legendgroup=featured_station,
                    showlegend=(i == 1)
                ),
                row=i, col=1
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=monthly_data['time'].tolist() + monthly_data['time'].tolist()[::-1],
                    ...
                    line=dict(color='rgba(255,255,255,0)'),
                    showlegend=False,
                    name=f'{featured_station} - {level} Std Dev'
                ),
                row=i, col=1
            )
    
    # Create dropdown options for non-featured stations
    dropdown_options = [{'label': 'Featured Stations', 'method': 'update', 'args': [{'visible': [True] * len(fig.data)}]}]
    
    for station in stations:
        if station not in featured_stations:
            ...
            for level in station_data['level'].unique():
                ...
                traces.append(
                    go.Scatter(
                        ...
                        line=dict(color='rgba(255,255,255,0)'),
                        showlegend=False,
                        name=f'{station} - {level} Std Dev',
                        visible=False
                    )
                )
            fig.add_traces(traces)
            
            # Add dropdown option for this station
            visibility = [False] * len(fig.data)
            for i in range(len(traces)):
                visibility[-(i+1)] = True
            dropdown_options.append({'label': station, 'method': 'update', 'args': [{'visible': visibility}]})

    # Update layout
    fig.update_layout(
        height=300 * len(featured_stations),
        updatemenus=[{
    ...
    for i in range(1, len(featured_stations) + 1):
...
featured_stations = ['DE-HoH', 'GF-Guy']  # change at will
fig = create_combined_interactive_plot(df, stations, featured_stations)
fig.show()
```
