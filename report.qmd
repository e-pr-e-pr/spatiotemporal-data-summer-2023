...
What makes this dataset in particular interesting is the levels in the SWC dataset. The SWC is measured at 4 different depts in the soil
...
def plot_grouped_stations(prefixes, grouped_data_list):
    # Calculate the number of rows needed
    num_plots = len(prefixes)
    num_rows = math.ceil(num_plots / 2)

    # Create a subplot figure with 2 columns and calculated number of rows
    fig = make_subplots(rows=num_rows, cols=2, 
                        subplot_titles=[f'Soil Water Content - Station {prefix}' for prefix in prefixes])
    
    for i, (prefix, grouped_data) in enumerate(zip(prefixes, grouped_data_list)):
        row = i // 2 + 1
        col = i % 2 + 1
    ...
    fig.update_layout(
        ...
        title_text="Soil Water Content for Multiple Stations",
        showlegend=True,
    ...
    for i in range(1, num_rows + 1):
        for j in range(1, 3):
            ...
            fig.update_yaxes(title_text="Soil Water Content", row=i, col=j)
