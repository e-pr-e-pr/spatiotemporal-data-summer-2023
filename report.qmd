...
def plot_grouped_stations(prefix, grouped_data):
    ...
    for station_key, station_data in grouped_data.items():
        ...
        for key, values in station_data.items():
            if key != 'time':
                ax.plot(times, values, label=f'{station_key} - {key}')
    
    ax.set_title(f'Soil Water Content - Station {prefix}')
    ax.set_xlabel('Time')
    ...
    # Instead of returning the figure, display it
    display(fig)
...
def create_combined_interactive_plot(df, stations, featured_stations):
    ...
    fig = make_subplots(rows=len(featured_stations), cols=1, 
                        subplot_titles=[f"Soil Water Content - {station}" for station in featured_stations])
    ...
    # Update layout
    fig.update_layout(
        height=300 * len(featured_stations),  # Adjust height based on number of featured stations
        updatemenus=[{
