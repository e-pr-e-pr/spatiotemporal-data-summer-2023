        ...
        for current_level in level_keys[1:]:
            ...
            while i < len(time_data[current_level]):
                ...
                if timestep not in time_data[reference_level]:
                   # print(f'Removing timestep {timestep} from {current_level}') # just for debugging and interest. Prints a lot! 
        ...
        for level in station_data['level'].unique():
            ...
            combined_fig.add_trace(
                go.Scatter(
                    ...
                    visible=(station in featured_stations)  # Only visible if it's a featured station
                )
            )
    ...
    combined_fig.update_layout(
        updatemenus=[
            dict(
                active=0,
                buttons=[
                    dict(label='Featured Stations',
                         method='update',
                         args=[{'visible': [station in featured_stations for station in stations for _ in df[df['station'] == station]['level'].unique()]}]),
                    *[dict(label=station,
                           method='update',
                           args=[{'visible': [s == station for s in stations for _ in df[df['station'] == s]['level'].unique()]}])
                      for station in stations]
                ],
                x=1.15,
                y=1.15,
        ...
        title="Soil Water Content - Interactive Plot",
        xaxis_title="Time",
