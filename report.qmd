...
def process_csv_file(file_path, column_to_analyze, output_dir):
    """
    Processes a CSV file to calculate daily averages and counts for a specified column.
    
    Parameters:
        file_path (str): Path to the input CSV file.
        column_to_analyze (str): The column for which to calculate daily averages. i.e. SWC_1, SWC_2 ...
    ...
    level = column_to_analyze.split('_')[1]
    
    df = pd.read_csv(file_path, parse_dates=['TIMESTAMP_START'])
    ...
    daily_stats = df.groupby(df.index)[column_to_analyze].agg(['mean', 'count'])
    
    # create the output file name
    output_file = f"{station_name}_swc_l{level}_daily_avg.txt"
    output_path = os.path.join(output_dir, output_file)
    
    # write results to the output file
    with open(output_path, 'w') as f:
        f.write(f"Daily averages and observation counts for {column_to_analyze}\n")
