...
print(swc_reco_merged_df.head())

def create_world_map(df, metric, level):
    level_data = df[df['level'] == level]
    
    fig = go.Figure(data=go.Scattergeo(
        lon = level_data['lon'],  # Use 'lon' for longitude
        lat = level_data['lat'],  # Use 'lat' for latitude
    ...
    fig.update_layout(
        title = f'{metric.capitalize()} between SWC and RECO for Level {level}, all station',
        geo = dict(
            showland = True,
            showcountries = True,
            showocean = True,
            ...
            landcolor = 'rgb(243, 243, 243)',
            oceancolor = 'rgb(230, 230, 250)',
            projection_type = 'natural earth',  # Use a natural earth projection for a nicer look
            showcoastlines = True,
            coastlinecolor = "RebeccaPurple",
            showframe = False,
            lonaxis=dict(showgrid=False),  # Turn off longitude gridlines
            lataxis=dict(showgrid=False)   # Turn off latitude gridlines
        ),
        autosize=True,  # Allow the figure to resize automatically
        width=800,  # Set width
        height=500,  # Set height
        margin=dict(l=0, r=0, t=30, b=0)  # Adjust margins to reduce white space
    )
...
for metric in metrics:
    for level in levels:
        fig = create_world_map(swc_reco_merged_df, metric, level)
        fig.show()
...
swc_reco_metrics_df = swc_reco_metrics_df.sort_values('station')
